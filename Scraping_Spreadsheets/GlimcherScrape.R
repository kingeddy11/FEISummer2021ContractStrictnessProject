...
path <- "C:/Users/edwar/OneDrive/Documents/FEISummer2021/CreditContractStrictnessProfBattaSum2021/Scraping/data/Guilloteau GLIMCHER REALTY TRUST.xlsx"
...
category_list<-c("EBITDA", "Net Income", "Total Debt", "Interest Charges", "Fixed Charges", "Cash Equivalents",
                 ...
                 "Affliate Transactions", "Borrowing", "Liens",  "Sales", 
                 "Distributions", "Covenant Components")
category_list
...
Glimcher_EBITDA_data_new<-add_new_columns(EBITDA_data_ori, "EBITDA", "Consolidated EBITDA")
Glimcher_Indebtedness_def_data_new<-add_new_columns(Indebtedness_def_data_ori, "Indebtedness Definition", "Indebtedness Definition")
Glimcher_Other_def_data_new<-add_new_columns(Other_def_data_ori, "Other Definitions", "Funds from operations")
Glimcher_Financial_Covenant_data_new<-add_new_columns(Financial_Covenant_data_ori, "Financial Covenants", c("consolidated net worth", NA, NA,
                                                                                                                  "Fixed charge ratio", NA, NA, "outstanding indebtedness"))
Glimcher_Capex_and_acqui_data_new<-add_new_columns(Capex_and_acqui_data_ori, "Capital Expenditures and Acquisitions", "Capital Expenditures and Acquisitions Definition")
Glimcher_Restricted_Payments_data_new<-add_new_columns(Restricted_Payments_data_ori, "Restricted Payments", "Restricted Payments Definition")

#selecting columns we want to keep
#EBITDA
Glimcher_EBITDA_data_new<-Glimcher_EBITDA_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Sign, `General category`, General_category_memo))

#Indebtedness Definition
Glimcher_Indebtedness_def_data_new <- Glimcher_Indebtedness_def_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Category))

#Financial Covenants
Glimcher_Financial_Covenant_data_new<-Glimcher_Financial_Covenant_data_new %>% 
  select(-c(Defintion))

#Other Definitions
Glimcher_Other_def_data_new<-Glimcher_Other_def_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text))
  
#Capital Expenditures and Acquisitions
Glimcher_Capex_and_acqui_data_new<-Glimcher_Capex_and_acqui_data_new %>% 
  select(-c(Definition))

#Restricted Payments
Glimcher_Restricted_Payments_data_new<-Glimcher_Restricted_Payments_data_new %>% 
  select(-c(Definition))
