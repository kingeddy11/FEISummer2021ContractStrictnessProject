...
path <- "C:/Users/edwar/OneDrive/Documents/FEISummer2021/CreditContractStrictnessProfBattaSum2021/Scraping/data/Hartmarx Corp.xlsx"
...
#EBITDA
EBITDA_data_ori$Sign[c(2:4)]<-1


#Indebtedness Definition
Indebtedness_def_data_ori$Sign[c(2:6)]<-1

#Net Income
Net_Income_data_ori$Sign[[1]]<-1
Net_Income_data_ori$Sign[c(2:6)]<--1
...
Hartmarx_EBITDA_data_new<-add_new_columns(EBITDA_data_ori, "EBITDA", "Consolidated EBITDA")
Hartmarx_Net_Income_data_new<-add_new_columns(Net_Income_data_ori, "Net Income", "Consolidated Net Income" )
Hartmarx_Total_Debt_data_new<-add_new_columns(Total_Debt_data_ori, "Total Debt", "Consolidated Total Debt")
Hartmarx_Cash_Equivalents_def_data_new<-add_new_columns(Cash_Equivalents_def_data_ori, "Cash Equivalents", "Cash Equivalents Definition")
Hartmarx_Indebtedness_def_data_new<-add_new_columns(Indebtedness_def_data_ori, "Indebtedness Definition", "Indebtedness Definition")
Hartmarx_Other_def_data_new<-add_new_columns(Other_def_data_ori, "Other Definitions","Other Definitions")
Hartmarx_Financial_Covenant_data_new<-add_new_columns(Financial_Covenant_data_ori, "Financial Covenants", "Financial Covenants Definition")
Hartmarx_Investments_data_new<-add_new_columns(Investments_data_ori, "Investments", "Investments Definition")
Hartmarx_Indebtedness_data_new<-add_new_columns(Indebtedness_data_ori, "Indebtedness", "Indebtedness")
Hartmarx_Liens_data_new<-add_new_columns(Liens_data_ori, "Liens", "Liens Definition")
Hartmarx_Disposals_data_new<-add_new_columns(Disposals_data_ori, "Disposals", "Disposals Definition")
Hartmarx_Capex_and_acqui_data_new<-add_new_columns(Capex_and_acqui_data_ori, "Capital Expenditures and Acquisitions", "Capital Expenditures and Acquisitions Definition")

#selecting columns we want to keep
#Capital Expenditures and Acquisitions
Hartmarx_Capex_and_acqui_data_new<-Hartmarx_Capex_and_acqui_data_new 

#Cash Equivalents
Hartmarx_Cash_Equivalents_def_data_new<-Hartmarx_Cash_Equivalents_def_data_new 
  
#Disposals
Hartmarx_Disposals_data_new<-Hartmarx_Disposals_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Category, Deduction))

#EBITDA
Hartmarx_EBITDA_data_new<-Hartmarx_EBITDA_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Sign, `General category`))
Hartmarx_EBITDA_data_new$Sign<-as.double(Hartmarx_EBITDA_data_new$Sign)

#Financial Covenants
Hartmarx_Financial_Covenant_data_new<-Hartmarx_Financial_Covenant_data_new 

#Indebtedness
Hartmarx_Indebtedness_data_new<-Hartmarx_Indebtedness_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Category, Deduction))

#Indebtedness Definition
Hartmarx_Indebtedness_def_data_new <- Hartmarx_Indebtedness_def_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Sign, Category))
Hartmarx_Indebtedness_def_data_new$Sign<-as.double(Hartmarx_Indebtedness_def_data_new$Sign)

#Investments
Hartmarx_Investments_data_new<-Hartmarx_Investments_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Category, Deduction))

#Liens
Hartmarx_Liens_data_new<-Hartmarx_Liens_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Category, Deduction))

#Net Income
Hartmarx_Net_Income_data_new<-Hartmarx_Net_Income_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Sign, `General category`))
Hartmarx_Net_Income_data_new$Sign<-as.double(Hartmarx_Net_Income_data_new$Sign)

#Other Definitions
Hartmarx_Other_def_data_new<-Hartmarx_Other_def_data_new %>% select(-c(`Specific Definition`)) %>% 
  rename(`Specific Definition`= Defintion) %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text))

#Total Debt
Hartmarx_Total_Debt_data_new<-Hartmarx_Total_Debt_data_new %>% 
