...
path <- "C:/Users/edwar/OneDrive/Documents/FEISummer2021/CreditContractStrictnessProfBattaSum2021/Scraping/data/GUILLOTEAU FORTUNE BRANDS.xlsx"
...
category_list<-c("EBITDA", "Net Income", "Total Debt", "Interest Charges", "Fixed Charges", "Cash Equivalents",
                 ...
                 "Affiliate Transactions", "Borrowing", "Liens",  "Sales", 
...
#Combining Affiliate Transactions and Acquisitions Sheets
#Capital Expenditures and Acquisitions
Affiliate_Transactions_uncomb_data_ori<-as_tibble(read_excel(path, col_names=TRUE, sheet=tab_names[[10]]))
Acquisitions_data_ori<-as_tibble(read_excel(path, col_names=TRUE, sheet=tab_names[[11]]))

Capex_and_acqui_data_ori<-full_join(Affiliate_Transactions_uncomb_data_ori, Acquisitions_data_ori)
rm(Affiliate_Transactions_uncomb_data_ori)
rm(Acquisitions_data_ori)
...
Fortune_Brands_EBITDA_data_new<-add_new_columns(EBITDA_data_ori, "EBITDA", "Consolidated EBITDA")
Fortune_Brands_Net_Income_data_new<-add_new_columns(Net_Income_data_ori, "Net Income", "Consolidated Net Income" )
Fortune_Brands_Total_Debt_data_new<-add_new_columns(Total_Debt_data_ori, "Total Debt", "Consolidated Indebtedness")
Fortune_Brands_Indebtedness_def_data_new<-add_new_columns(Indebtedness_def_data_ori, "Indebtedness Definition", "Indebtedness Definition")
Fortune_Brands_Financial_Covenant_data_new<-add_new_columns(Financial_Covenant_data_ori, "Financial Covenants", "Financial Covenants Definition")
Fortune_Brands_Indebtedness_data_new<-add_new_columns(Indebtedness_data_ori, "Indebtedness", "Indebtedness")
Fortune_Brands_Liens_data_new<-add_new_columns(Liens_data_ori, "Liens", "Liens Definition")
Fortune_Brands_Disposals_data_new<-add_new_columns(Disposals_data_ori, "Disposals", "Disposals Definition")
Fortune_Brands_Capex_and_acqui_data_new<-add_new_columns(Capex_and_acqui_data_ori, "Capital Expenditures and Acquisitions", "Capital Expenditures and Acquisitions Definition")

#selecting columns we want to keep
#EBITDA
Fortune_Brands_EBITDA_data_new<-Fortune_Brands_EBITDA_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Sign, `General category`, General_category_memo))

#Net Income
Fortune_Brands_Net_Income_data_new<-Fortune_Brands_Net_Income_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Sign, `General category`, General_category_memo))

#Total Debt
Fortune_Brands_Total_Debt_data_new<-Fortune_Brands_Total_Debt_data_new %>% 
  select(c(IntelligizeID, Item,`Specific Definition`, Text, `Main category`))

#Indebtedness Definition
Fortune_Brands_Indebtedness_def_data_new <- Fortune_Brands_Indebtedness_def_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Category))

#Financial Covenants
Fortune_Brands_Financial_Covenant_data_new<-Fortune_Brands_Financial_Covenant_data_new

#Indebtedness
Fortune_Brands_Indebtedness_data_new<-Fortune_Brands_Indebtedness_data_new %>% 
  select(-c(Definition))

#Liens
Fortune_Brands_Liens_data_new<-Fortune_Brands_Liens_data_new %>% select(-c(Definition))

#Disposals
Fortune_Brands_Disposals_data_new<-Fortune_Brands_Disposals_data_new %>% select(-c(Definition))

#Capital Expenditures and Acquisitions
Fortune_Brands_Capex_and_acqui_data_new<-Fortune_Brands_Capex_and_acqui_data_new %>% 
  select(-c(Definition))
