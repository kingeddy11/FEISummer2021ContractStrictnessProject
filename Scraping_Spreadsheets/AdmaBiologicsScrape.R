...
path <- "C:/Users/edwar/OneDrive/Documents/FEISummer2021/CreditContractStrictnessProfBattaSum2021/Scraping/data/Karasinski Adma Biologics.xlsx"
...
Adma_Biologics_Cash_Equivalents_def_data_new<-add_new_columns(Cash_Equivalents_def_data_ori, "Cash Equivalents", "Cash Equivalents Definition")
Adma_Biologics_Indebtedness_def_data_new<-add_new_columns(Indebtedness_def_data_ori, "Indebtedness Definition", "Indebtedness Definition")
Adma_Biologics_Financial_Covenant_data_new<-add_new_columns(Financial_Covenant_data_ori, "Financial Covenants", "Financial Covenants Definition")
Adma_Biologics_Investments_data_new<-add_new_columns(Investments_data_ori, "Investments", "Investments Definition")
Adma_Biologics_Liens_data_new<-add_new_columns(Liens_data_ori, "Liens", "Liens Definition")
Adma_Biologics_Disposals_data_new<-add_new_columns(Disposals_data_ori, "Disposals", "Disposals Definition")


#selecting columns we want to keep
#Cash Equivalents
Adma_Biologics_Cash_Equivalents_def_data_new<-Adma_Biologics_Cash_Equivalents_def_data_new %>%
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Category, Deduction))

#Indebtedness Definition
Adma_Biologics_Indebtedness_def_data_new<-Adma_Biologics_Indebtedness_def_data_new %>% 
  select(-c(definition))

#Financial Covenants
Adma_Biologics_Financial_Covenant_data_new<-Adma_Biologics_Financial_Covenant_data_new %>% 
  select(-c(Category)) %>% rename(Category=definition)

#Investments
Adma_Biologics_Investments_data_new<-Adma_Biologics_Investments_data_new %>%
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Category, Deduction))
  
#Liens
Adma_Biologics_Liens_data_new<-Adma_Biologics_Liens_data_new %>% 
  select(-c(definition))

#Disposals
Adma_Biologics_Disposals_data_new<-Adma_Biologics_Disposals_data_new %>% 
  select(-c(definition))
