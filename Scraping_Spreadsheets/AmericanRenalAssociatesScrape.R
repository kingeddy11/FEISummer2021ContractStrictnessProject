...
path <- "C:/Users/edwar/OneDrive/Documents/FEISummer2021/CreditContractStrictnessProfBattaSum2021/Scraping/data/Karasinski American Renal Associates.xlsx"
...
category_list<-c("EBITDA", "NI", "Total Debt", "Interest Charges", "Fixed Charge", "Cash Equivalents", "Indebtedness Def", "Other", 
                 ...
                 "Debt", "Liens",  "Asset Disposition", "Distribution", "Covenant Components")
category_list
...
Indebtedness_def_data_ori<-Indebtedness_def_data_ori %>% drop_na(Text)
...
#Total Debt
Total_Debt_data_ori<-Total_Debt_data_ori %>% drop_na(Text)
...
American_Renal_EBITDA_data_new<-add_new_columns(EBITDA_data_ori, "EBITDA", "Consolidated EBITDA")
American_Renal_Net_Income_data_new<-add_new_columns(Net_Income_data_ori, "Net Income", "Adjusted Net Income")
American_Renal_Total_Debt_data_new<-add_new_columns(Total_Debt_data_ori, "Total Debt", "Consolidated Total Debt")
American_Renal_Cash_Equivalents_def_data_new<-add_new_columns(Cash_Equivalents_def_data_ori, "Cash Equivalents", "Cash Equivalents Definition")
American_Renal_Indebtedness_def_data_new<-add_new_columns(Indebtedness_def_data_ori, "Indebtedness Definition", "Indebtedness Definition")
American_Renal_Financial_Covenant_data_new<-add_new_columns(Financial_Covenant_data_ori, "Financial Covenants", "Financial Covenants Definition")
American_Renal_Investments_data_new<-add_new_columns(Investments_data_ori, "Investments", "Investments Definition")
American_Renal_Liens_data_new<-add_new_columns(Liens_data_ori, "Liens", "Liens Definition")
American_Renal_Disposals_data_new<-add_new_columns(Disposals_data_ori, "Disposals", "Disposals Definition")

#selecting columns we want to keep
#EBITDA
American_Renal_EBITDA_data_new<-American_Renal_EBITDA_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Sign, `General category`, General_category_memo))

#Net Income
American_Renal_Net_Income_data_new<-American_Renal_Net_Income_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Sign, `General category`, General_category_memo))

#Total Debt
American_Renal_Total_Debt_data_new<-American_Renal_Total_Debt_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Sign, `General category`, General_category_memo))

#Cash Equivalents
American_Renal_Cash_Equivalents_def_data_new<-American_Renal_Cash_Equivalents_def_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Category, Deduction))

#Indebtedness Definition
American_Renal_Indebtedness_def_data_new<-American_Renal_Indebtedness_def_data_new %>% rename(Category=category) %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Category))

#Financial Covenants
American_Renal_Financial_Covenant_data_new<-American_Renal_Financial_Covenant_data_new
American_Renal_Financial_Covenant_data_new$Threshold<-as.character(American_Renal_Financial_Covenant_data_new$Threshold)

#Investments
American_Renal_Investments_data_new<-American_Renal_Investments_data_new %>% 
  select(-c(definition))

#Liens
American_Renal_Liens_data_new<-American_Renal_Liens_data_new %>% 
  select(-c(definition))

#Disposals
American_Renal_Disposals_data_new<-American_Renal_Disposals_data_new %>% 
  select(-c(definition))
