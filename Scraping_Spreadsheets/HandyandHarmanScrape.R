...
path <- "C:/Users/edwar/OneDrive/Documents/FEISummer2021/CreditContractStrictnessProfBattaSum2021/Scraping/data/Handy and Harman Ltd.xlsx"
...
category_list<-c("EBITDA", "Net Income", "Total Debt", "Interest Charges", "Fixed Charges", "Cash Equivalents",
                 "Debt", "Other", "Financial Cov", "Dynamic Threshold", "Investments", 
                 "M&A", "Indebtedness$", "Liens",  "Asset Sales", 
...
#separately combining M&A and Capex dataset to Capex
#Negative Cov
MA_data_ori<-as_tibble(read_excel(path, col_names=TRUE, sheet=tab_names[[10]]))
Capex_data_ori<-as_tibble(read_excel(path, col_names=TRUE, sheet=tab_names[[14]]))

Capex_and_acqui_data_ori<-full_join(MA_data_ori, Capex_data_ori)
rm(MA_data_ori)
rm(Capex_data_ori)
...
Cash_Equivalents_def_data_ori<-Cash_Equivalents_def_data_ori %>% rename(Text=1) %>% 
  rename(Category=2) %>% rename(Deduction=3)
Cash_Equivalents_def_data_ori$Deduction[[1]]<-NA

#EBITDA
EBITDA_data_ori$Sign[c(2:11)]<-1


#Indebtedness Definition
Indebtedness_def_data_ori$Sign[c(2:12)]<-1
...
Handy_and_Harman_EBITDA_data_new<-add_new_columns(EBITDA_data_ori, "EBITDA", "Consolidated EBITDA")
Handy_and_Harman_Cash_Equivalents_def_data_new<-add_new_columns(Cash_Equivalents_def_data_ori, "Cash Equivalents", "Cash Equivalents Definition")
Handy_and_Harman_Indebtedness_def_data_new<-add_new_columns(Indebtedness_def_data_ori, "Indebtedness Definition", "Indebtedness Definition")
Handy_and_Harman_Other_def_data_new<-add_new_columns(Other_def_data_ori, "Other Definitions","Other Definitions")
Handy_and_Harman_Financial_Covenant_data_new<-add_new_columns(Financial_Covenant_data_ori, "Financial Covenants", "Financial Covenants Definition")
Handy_and_Harman_Investments_data_new<-add_new_columns(Investments_data_ori, "Investments", "Investments Definition")
Handy_and_Harman_Indebtedness_data_new<-add_new_columns(Indebtedness_data_ori, "Indebtedness", "Indebtedness")
Handy_and_Harman_Liens_data_new<-add_new_columns(Liens_data_ori, "Liens", "Liens Definition")
Handy_and_Harman_Disposals_data_new<-add_new_columns(Disposals_data_ori, "Disposals", "Disposals Definition")
Handy_and_Harman_Capex_and_acqui_data_new<-add_new_columns(Capex_and_acqui_data_ori, "Capital Expenditures and Acquisitions", "Capital Expenditures and Acquisitions Definition")
Handy_and_Harman_Restricted_Payments_data_new<-add_new_columns(Restricted_Payments_data_ori, "Restricted Payments", "Restricted Payments Definition")

#selecting columns we want to keep
#Capital Expenditures and Acquisitions
Handy_and_Harman_Capex_and_acqui_data_new<-Handy_and_Harman_Capex_and_acqui_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Category, Deduction))

#Cash Equivalents
Handy_and_Harman_Cash_Equivalents_def_data_new<-Handy_and_Harman_Cash_Equivalents_def_data_new
  
#Disposals
Handy_and_Harman_Disposals_data_new<-Handy_and_Harman_Disposals_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Category, Deduction))

#EBITDA
Handy_and_Harman_EBITDA_data_new<-Handy_and_Harman_EBITDA_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Sign, `General category`))
Handy_and_Harman_EBITDA_data_new$Sign<-as.double(Handy_and_Harman_EBITDA_data_new$Sign)

#Financial Covenants
Handy_and_Harman_Financial_Covenant_data_new<-Handy_and_Harman_Financial_Covenant_data_new %>% 
  select(-c(Conditions))

#Indebtedness
Handy_and_Harman_Indebtedness_data_new<-Handy_and_Harman_Indebtedness_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Category, Deduction))

#Indebtedness Definition
Handy_and_Harman_Indebtedness_def_data_new <- Handy_and_Harman_Indebtedness_def_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Sign, Category))
Handy_and_Harman_Indebtedness_def_data_new$Sign<-as.double(Handy_and_Harman_Indebtedness_def_data_new$Sign)

#Investments
Handy_and_Harman_Investments_data_new<-Handy_and_Harman_Investments_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Category, Deduction))

#Liens
Handy_and_Harman_Liens_data_new<-Handy_and_Harman_Liens_data_new %>% 
  select(-c(Description, Conditions))

#Other Definitions
Handy_and_Harman_Other_def_data_new<-Handy_and_Harman_Other_def_data_new %>% select(-c(`Specific Definition`)) %>% 
  rename(`Specific Definition`= Defintion) %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text))

#Restricted Payments
Handy_and_Harman_Restricted_Payments_data_new<-Handy_and_Harman_Restricted_Payments_data_new %>%
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Category, Deduction))
