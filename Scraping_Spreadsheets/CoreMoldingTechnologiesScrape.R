...
path <- "C:/Users/edwar/OneDrive/Documents/FEISummer2021/CreditContractStrictnessProfBattaSum2021/Scraping/data/Core Molding Technologies Inc.xlsx"
...
Indebtedness_def_data_ori$Sign<-c(NA,1,1,1,1,1,1)
...
Core_Molding_Tech_Indebtedness_def_data_new<-add_new_columns(Indebtedness_def_data_ori, "Indebtedness Definition", "Indebtedness Definition")
Core_Molding_Tech_Other_def_data_new<-add_new_columns(Other_def_data_ori, "Other Definitions","Core Composites")
Core_Molding_Tech_Financial_Covenant_data_new<-add_new_columns(Financial_Covenant_data_ori, "Financial Covenants", "Financial Covenants Definition")
Core_Molding_Tech_Investments_data_new<-add_new_columns(Investments_data_ori, "Investments", "Investments Definition")
Core_Molding_Tech_Indebtedness_data_new<-add_new_columns(Indebtedness_data_ori, "Indebtedness", "Indebtedness")
Core_Molding_Tech_Liens_data_new<-add_new_columns(Liens_data_ori, "Liens", "Liens Definition")
Core_Molding_Tech_Disposals_data_new<-add_new_columns(Disposals_data_ori, "Disposals", "Disposals Definition")
Core_Molding_Tech_Capex_and_acqui_data_new<-add_new_columns(Capex_and_acqui_data_ori, "Capital Expenditures and Acquisitions", "Capital Expenditures and Acquisitions Definition")

#selecting columns we want to keep
#Capital Expenditures and Acquisitions
Core_Molding_Tech_Capex_and_acqui_data_new<-Core_Molding_Tech_Capex_and_acqui_data_new %>% 
  select(-c(Definition, Conditions))

#Disposals
Core_Molding_Tech_Disposals_data_new<-Core_Molding_Tech_Disposals_data_new %>% 
  select(-c(Conditions))

#Financial Covenants
Core_Molding_Tech_Financial_Covenant_data_new<-Core_Molding_Tech_Financial_Covenant_data_new %>% 
  select(-c(Conditions))

#Indebtedness
Core_Molding_Tech_Indebtedness_data_new<-Core_Molding_Tech_Indebtedness_data_new %>% 
  select(-c(Conditions))

#Indebtedness Definition
Core_Molding_Tech_Indebtedness_def_data_new <- Core_Molding_Tech_Indebtedness_def_data_new %>% 
  select(c(IntelligizeID, Item, `Specific Definition`, Text, Sign, Category))

#Investments
Core_Molding_Tech_Investments_data_new<-Core_Molding_Tech_Investments_data_new %>% 
  select(-c(Definitions, Conditions))

#Liens
Core_Molding_Tech_Liens_data_new<-Core_Molding_Tech_Liens_data_new %>% 
  select(-c(Definition, Conditions))

#Other Definitions
Core_Molding_Tech_Other_def_data_new<-Core_Molding_Tech_Other_def_data_new %>% 
